...
"""
 ...
 https://raw.githubusercontent.com/MisterDaneel/pysoxy/master/pysoxy.py
"""
...
def request_client(wrapper):
	...
	logging.debug('request_client()')

	try:
		logging.debug('request_client recv()...')
		s5_request = wrapper.recv(BUFSIZE)
		logging.debug('request_client recv() done')
	...
	logging.debug('request_client s5_request=%s' % (s5_request))
	...
	# DOMAIN NAME
		...
		logging.error('bad s5_request')
		return False

	logging.info('destination: %s:%s' % (dst_addr, dst_port))
...
def connection(wrapper):
	...
	if subnegotiation(wrapper):
		logging.debug('subnegotiation complete')
		request(wrapper)
...
def bind_port(sock):
	...
	"""
	try:
		logging.debug('Bind {}'.format(str(LOCAL_PORT)))
...
def exit_handler(signum, frame):
	...
	logging.debug('Signal handler called with signal', signum)
	EXIT.set_status(True)
...
class STDIO_connection:
	...
	def sendall(self, bytes):
		logging.debug('sendall %d bytes' % (len(bytes)))
		sys.stdout.buffer.write(bytes)
		sys.stdout.flush()

	def send(self, bytes):
		logging.debug('send %d bytes' % (len(bytes)))
		sys.stdout.buffer.write(bytes)
		sys.stdout.flush()
...
# socat tcp-listen:1080,fork,reuseaddr exec:%
